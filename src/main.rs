...
#[derive(Clone, Debug, Serialize)]
struct DestinationDirectModelArtifact {
    matrix_role: &'static str,
    matrix_sha256: String,
    distance: f64,
...
fn analyze_bridge_validated(
    ...
    let static_distance_index = static_destination_matrix
        .as_ref()
        .map(|(matrix, _)| FixedMatrixDistanceIndex::new(bridge_tracks, matrix));
    let learned_distance_index = (destination_route
    ...
    let distance_index = destination_distance_selection
        ...
        .map(|(selected_role, _, _, _)| match selected_role {
            ...
            DestinationMatrixRole::Static => static_distance_index
                .as_ref()
                .expect("the Static destination role has a distance index"),
        });
    ...
    for position in first_gap..selected_library_route.len() {
        ...
        let (direct_distance, direct_percentile) = destination_distance_selection
            ...
            .map(|(_, _, quality, _)| {
                let leg = quality
                    .legs
                    .get(destination_leg)
                    .expect("the destination direct route contains every locked edge");
                (leg.2, leg.3)
            })
            .unwrap_or((gap.direct_distance, gap.direct_percentile));
        ...
        gaps.push(BridgeGapArtifact {
            ...
            direct_distance,
            direct_percentile,
            triggering: trigger_percentile.map(|threshold| direct_percentile > threshold),
    ...
    let (context_policy, seed_policy) = match request.extension.mode.as_str() {
        "destination_route" => (
            ...
            "recent immutable listening history plus the locked route start",
        ),
        "fixed_source_extension" => (
    ...
    if request.extension.mode == "destination_route" {
        ...
        let anchors_valid = if let Some(rejoin) = rejoin {
            let rejoin_position = request
                .source_tracks
                .iter()
                .position(|track| track.id == rejoin)
                .ok_or_else(|| {
                    CommandFailure::new(
                        "DESTINATION_ROUTE_REJOIN_INVALID",
